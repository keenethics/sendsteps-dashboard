...
class Translations_Model extends Model {
    public function findTranslations($languagesRequested = array()){
        if (count($languagesRequested) != 0){
            $query = "SELECT tk.id AS keyId, tk.text AS `key`, t.text, l.name AS `language` FROM translations t LEFT JOIN languages l ON l.id = t.languageId LEFT JOIN translation_keys tk ON t.keyId = tk.id WHERE <l.name> IN ( :langSet )";
            ...
            $query = "SELECT tk.id AS keyId, tk.text AS `key`, t.text, l.name AS `language` FROM translations t LEFT JOIN languages l ON l.id = t.languageId LEFT JOIN translation_keys tk ON t.keyId = tk.id" ;
